    ...
    <div key={user.id} className="flex flex-row mb-4 justify-between">
      <div className="flex items-center mb-2">
        {user.FollowingProfile.User.profile && user.FollowingProfile.User.profile.photoUrl ? (
          ...
          <FaRegUserCircle className="w-16 h-16 max-sm:w-12 max-sm:h-12 rounded-full mr-2 text-blue-900" />
       )}
        <div className='sm:border-4 border-blue-900 py-2 text-blue-900 px-5 rounded-lg 
        max-sm:px-2 '>
         ...
         {isFollowing ? (
          <button
            className="text-slate-50 hover:underline  px-10 max-sm:h-10 h-14 max-sm:px-2 
            bg-red-700 ml-5 rounded text-lg md:text-2xl" 
