...
const OrderBanner: React.FC = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        const response = await fetch('http://localhost:3500/orderRequest/banners', {
          headers: {
  ...
  if (orders.length === 0) {
    ...
    font-bold max-sm:text-xl text-indigo-900   mb-4'>No orders available for product banner.</div>;
  }

  return (
    <div className="mt-8 text-xl ">
      <h2 className="flex justify-center text-2xl lg:text-3xl xl:text-4xl font-bold
       max-sm:text-xl text-indigo-900   mb-4">Order Request for  product Banners</h2>
