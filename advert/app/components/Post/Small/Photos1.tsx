...
const Photos1: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        //@ts-ignore
        const response: ImageResponse = await getImage(filter, searchQuery);
      ...
      {isLoading ? (
        ...
        <>
          {Object.keys(groupedProducts).length > 0 && (
            <ul className='grid grid-cols-4 max-xl:grid-cols-2 h-auto overflow-y-auto max-sm:grid-cols-1 max-lg:grid-cols-2 max-md:grid-cols-2 justify-between gap-2'>
              {Object.keys(groupedProducts).map((productId) => {
                ...
                return (
                  ...
                  >
                    ...
                    />
                    <DeleteImages productId={product.id} />
                    </h1>
                    </span>
          ...
          {Object.keys(groupedProducts).length === 0 && <p className='flex justify-center lg:text-5xl md:text-3xl h-screen max-md:text-2xl text-blue-950'>
