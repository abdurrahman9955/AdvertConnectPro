...
export const likeVideo = async (productId: string, ): Promise<ApiResponse> => {
  try {
    ...
    const response: AxiosResponse<ApiResponse> = await axios.post(
      `${API_BASE_URL}/likes/video/${productId}`,
      {},
      {
        headers: {
...
export const getLikeVideos = async (productId: string): Promise<number> => {
  try {
