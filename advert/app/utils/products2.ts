...
export const getProductById2 = async (productId: number): Promise<Product | null> => {
  try {

    const response: AxiosResponse<ApiResponse> = await axios.get(`${API_BASE_URL}/product2/${productId}`,
...
export const updateProduct2 = async (productId: number, updatedData: Partial<Product>): Promise<ApiResponse> => {
  try {
    ...
    const response: AxiosResponse<ApiResponse> = await axios.put(
      `${API_BASE_URL}/product2/${productId}`,
      updatedData,
      {
        headers: {
          Authorization: `Bearer ${token}`,
