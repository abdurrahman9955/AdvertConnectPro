...
export const bannerOrder = async (order: Order,): Promise<ApiResponse> => {
  try {
    ...
    const response: AxiosResponse<ApiResponse> = await axios.post(
      `${API_BASE_URL}/orderRequest/banner`,
      order,
      {
        headers: {
...
export const getBannerOrders = async (): Promise<ApiResponse> => {
  try {
    ...
    axios.get(`${API_BASE_URL}/orderRequest/banners`, {
      headers: {
        Authorization: `Bearer ${token}`,
        'user-id': userId,
      },
