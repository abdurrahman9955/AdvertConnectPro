...
const SharingImages: React.FC<ProductSharingProps> = ({ productName, productUrl, productImageUrl }) => {
  ...
  return (
    <div>
      ...
      {showSharingImages && (
        <div className="fixed bg-slate-200 flex justify-between p-4 gap-5 flex-wrap border-4 border-slate-950
        top-0 left-0 w-auto h-auto">

          <button className='flex flex-1'
          onClick={closeShare}>
          <FaRegWindowClose size={50} color='dark' />
          </button>

          <button 
          onClick={() => shareOnSocialMedia('facebook')}><FaFacebook size={50} color='darkblue' /></button>

          <button 
          onClick={() => shareOnSocialMedia('twitter')}><FaTwitter size={50} color='darkcyan'/></button>

          <button 
          onClick={() => shareOnSocialMedia('instagram')}><FaInstagram size={50} color='darkred'/></button>

          <button 
          onClick={() => shareOnSocialMedia('email')}><FaEnvelope size={50} color='darkred'/></button>

          <button 
          onClick={() => shareOnSocialMedia('whatsapp')}><FaWhatsappSquare  size={50} color='darkgreen'/></button>

          <button 
          onClick={() => shareOnSocialMedia('telegram')}><FaTelegram size={50} color='Blue'/></button>

          <button 
          onClick={() => shareOnSocialMedia('linkedin')}><FaLinkedin size={50} color='darkblue'/></button>

          <button 
          onClick={() => shareOnSocialMedia('pinterest')}><FaPinterest size={50} color='darkred'/></button>
