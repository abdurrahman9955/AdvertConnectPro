...
import { getImagesRatings } from '@/app/utils/rateImages';
import Cookies from 'js-cookie';


const ViewRating = ({ productId }: { productId: string }) => {
  ...
  useEffect(() => {
    ...
    const fetchRatings = async () => {
      try {
        const ratings = await getImagesRatings(productId);
  ...
  const renderAverageRating = () => {
    ...
    for (let i = 1; i <= 5; i++) {
      if (i <= roundedRating) {
        ...
        stars.push(<FaStarHalf key={i} color="gold" size={20} />);
      } else {
        stars.push(<FaRegStar key={i} color="gray" size={20} />);
      }
    }
    return stars;
  };
