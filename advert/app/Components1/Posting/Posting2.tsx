...
const ProductBanner: React.FC = () => {
  ...
  return (
    <div className='font-bold text-xl min-h-screen'>
      {isLoading ? (
        <p className='lg:text-5xl md:text-3xl h-screen max-md:text-2xl text-blue-950'>
          Loading product Banners...
        </p>
      ) : (
        <>
          {currentProducts.length > 0 && (
            <ul className='grid grid-cols-2 h-auto max-sm:grid-cols-1 max-lg:grid-cols-2 max-md:grid-cols-2 gap-2'>
                ...
                <li key={product.id} onClick={() => handleProductClick(product.id)} className='border-4 border-slate-950 bg-white p-5'>
                  ...
                  {product.mediaType === 'BANNER' && (
                    <div className='relative'>
                      <img src={product.mediaUrl} alt={`Product banner for ${product.productName || 'Product'}`} width={300} height={300} className='w-full border-2 h-96 border-slate-900 rounded-lg' />
                  ...
                  <div className='flex flex-row flex-wrap justify-between'>
                    <p>{product.country ? product.country : " was not provided"}</p>
                    <p className='overflow-hidden'>Company Name: {product.companyName ? product.companyName : " was not provided"}</p>
          ...
          {currentProducts.length === 0 && <p className='lg:text-5xl 
          md:text-3xl h-screen max-md:text-2xl
          ...
          <div className='flex justify-center mt-4'>
            {Array.from({ length: totalPages }, (_, index) => (
              ...
              'bg-blue-900 text-white rounded-md border-2 border-white' : 
