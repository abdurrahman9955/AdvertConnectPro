    ...
    const fetchProducts = async () => {
      try {
        ...
        if (response.success && Array.isArray(response.banners)) {
          const groceryProducts = response.banners.filter(product => product.product === 'Babies & Kids');
      ...
      {isLoading ? (
       ...
       max-md:text-2xl text-blue-950'>Loading products...</p>
        ...
        <>
          ...
          {Object.keys(groupedProducts).length === 0 && <p className='lg:text-4xl md:text-3xl h-screen max-md:text-2xl text-blue-950'>
          No products for Babies & kids with banner found!
        </p>}
        </>
      )}
    </div>
