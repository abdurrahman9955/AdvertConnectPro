...
import { updateFilter2, resetFilter2  } from '@/app/app/filters/filter2'; 
...
const Animals3 = () => {
  ...
  useEffect(() => {
    const storedFilter = Cookies.get('filter2');
    if (storedFilter) {
  ...
  const handleSubmit = () => {
    ...
    dispatch(updateFilter2(parsedFilter));
    Cookies.set('filter2', JSON.stringify(parsedFilter), { expires: 1, path: '/' });
