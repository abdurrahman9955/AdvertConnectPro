...
const Recipe = () => {
  ...
  const initialRoute = localStorage.getItem('selectedRoute') || 'Recipe1'
    ...
    <div className=" w-full ">
      <div className="border-4 border-indigo-900  py-3 w-full  ">
        <div className="flex justify-items-center  max-lg:justify-between overflow-x-auto">
               ...
               <button onClick={() => setSelectedRoute('Recipe1')}
                type="button" className="text-lime-100 bg-indigo-950
                ml-3 rounded-xl h-10  w-auto pl-5 pr-5 mr-5 text-xl
                lg:px-7 hover:bg-neutral-700  ">Recipe</button>
             ...
             <button  onClick={handleSearch}  className='text-3xl text-blue-950  sm:hidden mr-5 '>

            {SearchVisible ? (<span  onClick={closeSearch}  ><Back  /></span>  )
             : ( <FaSearch  />  )} </button>

            <div> {SearchVisible &&  (<Search />) }</div>

               <button onClick={() => setSelectedRoute('Recipe3')}
                 type="button" className="text-lime-100 bg-indigo-950
                ml-3 rounded-xl h-10    mr-2  w-auto pl-5 pr-5 max-sm:py-2 hover:bg-neutral-700 ">Filters</button>
           ...
           {selectedRoute === 'Recipe1' && <Recipe1 />}
           {selectedRoute === 'Recipe2' && <Recipe2 />}
           {selectedRoute === 'Recipe3' && <Recipe3 />}
