    ...
    <div className="w-full ">
        ...
        <div>
            <div className="flex justify-between  gap-4 overflow-x-auto 
             ...
             py-2 border-4 border-purple-950  " >


               <button  onClick={() => setSelectedRoute('PrivacyUse')}
              type="submit" className="h-7 max-sm:px-3 max-sm:h-6 px-10 ml-4 hover:bg-indigo-700 
               rounded-sm bg-blue-950 text-slate-50">UseOfInformation
               </button>

               <button onClick={() => setSelectedRoute('PrivacyInfo')}
              type="submit" className="h-7 max-sm:px-3 px-10 ml-4 hover:bg-indigo-700 
               rounded-sm max-sm:h-6  bg-blue-950 text-slate-50">InformationWeCollect
               </button>

              <button onClick={() => setSelectedRoute('PrivacySecurity')}
               type="submit" className="h-7 max-sm:px-3 px-10 hover:bg-indigo-700 
              rounded-sm max-sm:h-6  bg-blue-950 text-slate-50">DataSecurity
              </button>
              
              <button onClick={() => setSelectedRoute('PrivacyShare')}
              type="submit" className="h-7 max-sm:px-3 px-10 hover:bg-indigo-700 
              rounded-sm max-sm:h-6  bg-blue-950 text-slate-50">DataSharing
              </button>

              <button onClick={() => setSelectedRoute('PrivacyRights')}
              type="submit" className="h-7 max-sm:px-3 px-10 hover:bg-indigo-700
               rounded-sm max-sm:h-6   bg-blue-950 text-slate-50">UserRights
               </button> 
              
              <button onClick={() => setSelectedRoute('PrivacyChange')}
               type="submit" className="h-7 max-sm:px-3 px-10 hover:bg-indigo-700 
              rounded-sm max-sm:h-6  bg-blue-950 text-slate-50">ChangesToPrivacyPolicy
              </button>
             
              <button onClick={() => setSelectedRoute('PrivacyContact')}
              type="submit" className="h-7 max-sm:px-3 px-10 hover:bg-indigo-700 
              rounded-sm max-sm:h-6  mr-4 bg-blue-950 text-slate-50">ContactUs
              </button>
