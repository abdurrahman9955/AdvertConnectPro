    ...
    const fetchProducts = async () => {
      try {
        ...
        if (response.success && Array.isArray(response.videos)) {
          const groceryProducts = response.videos.filter(product => product.product === 'Electronics');
  ...
  return (
    <div className='font-bold text-sm w-full min-h-screen overflow-hidden' >
      {isLoading ? (
        ...
        <>
          {products.length > 0 && (
            <ul className='grid grid-cols-4 h-auto overflow-y-auto max-sm:grid-cols-1
             max-lg:grid-cols-2 max-md:grid-cols-2 justify-between gap-1'>
              {products.map((product) => (
               
               
                  <li  key={product.id}
                    onClick={() => handleProductClick(product.id)}
                    className='border-2 border-slate-950 bg-white p-sm'
                  > 
                    ...
                    <button 
                      className='w-full h-7 text-white mt-3 rounded-sm bg-slate-950
                       hover:bg-blue-900'
                    >
                      <h1>watch video</h1>
                    </button>
          ...
          {products.length === 0 &&  <p className='flex justify-center lg:text-2xl 
          md:text-xl h-screen max-md:text-lg text-blue-950'>
          No ELectronics with video found!
        </p>}
        </>
      )}
    </div>
