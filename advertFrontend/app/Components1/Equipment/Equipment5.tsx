    ...
    const fetchProducts = async () => {
      try {
        ...
        if (response.success && Array.isArray(response.videos)) {
          const groceryProducts = response.videos.filter(product => product.product === 'Equipments');
    ...
    <div className='font-bold text-sm w-full min-h-screen overflow-hidden' >
      {isLoading ? (
        ...
        <>
          ...
          md:text-xl h-screen max-md:text-sm text-blue-950'>
          No Equipments with video found!
        </p>}
        </>
      )}
    </div>
