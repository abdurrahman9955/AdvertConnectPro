    ...
    const fetchProducts = async () => {
      try {
        ...
        if (response.success && Array.isArray(response.videos)) {
          const groceryProducts = response.videos.filter(product => product.product === 'Coffee Shops');
    ...
    <div className='font-bold text-sm w-full min-h-screen overflow-hidden' >
      {isLoading ? (
        ...
        <>
          ...
          {products.length === 0 &&  <p className='flex justify-center
           lg:text-2xl md:text-xl h-screen max-md:text-lg text-blue-950'>
          No CoffeeShops with video found!
        </p>}
        </>
      )}
    </div>
