...
const Kitchen5: React.FC = () => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        if (response.success && Array.isArray(response.videos)) {
          const groceryProducts = response.videos.filter(product => product.product === 'Kitchen');
  ...
  return (
    <div className='font-bold text-sm w-full min-h-screen overflow-hidden' >
      {isLoading ? (
        <p className='flex justify-center lg:text-2xl md:text-xl h-screen max-md:text-sm
         text-blue-950'>
          Loading products...
        </p>
      ) : (
                  ...
                  > 
                    ...
                    <div className='flex justify-between flex-wrap'>
                      <button className='flex flex-row bg-blue-50 border
                       border-black rounded-sm mt-2 p-1'>
                    ...
                    <button 
                      className='w-full h-8 text-white mt-3 rounded-sm bg-slate-950
                       hover:bg-blue-900'
                    >
                      <h1>watch video</h1>
                    </button>
