...
const Books5: React.FC = () => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        if (response.success && Array.isArray(response.videos)) {
          const groceryProducts = response.videos.filter(product => product.product === 'Books');
  ...
  return (
    <div className='font-bold text-sm w-full min-h-screen overflow-hidden' >
      {isLoading ? (
        <p className='flex justify-center lg:text-2xl md:text-xl 
        h-screen max-md:text-lg text-blue-950'>
          Loading products...
        </p>
      ) : (
        <>
             ...
             max-lg:grid-cols-2 max-md:grid-cols-2 justify-between gap-1'>
              {products.map((product) => (
                  ...
                  > 
                    ...
                    <p className='overflow-hidden flex justify-center'>
                    ...
                    <button 
                      className='w-full h-8 text-white mt-3 rounded-sm bg-slate-950 hover:bg-blue-900'
                    >
                      <h1>watch video</h1>
                    </button>
                    </Link>
