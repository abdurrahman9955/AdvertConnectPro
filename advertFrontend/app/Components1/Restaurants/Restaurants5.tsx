    ...
    const fetchProducts = async () => {
      try {
        ...
        if (response.success && Array.isArray(response.videos)) {
          const groceryProducts = response.videos.filter(product => product.product === 'Restaurants');
    ...
    <div className='font-bold text-sm w-full min-h-screen overflow-hidden' >
      {isLoading ? (
        ...
        <>
            ...
            max-lg:grid-cols-2 max-md:grid-cols-2 justify-between gap-1'>
              {products.map((product) => (
                  ...
                  > 
                    ...
                    <button 
                      className='w-full h-7 text-white mt-3 rounded-sm bg-slate-950 hover:bg-blue-900'
                    >
                      <h1>watch video</h1>
                    </button>
                    </Link>
          ...
          No Restaurants with video found!
        </p>}
        </>
      )}
    </div>
