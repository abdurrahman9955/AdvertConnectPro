...
const UserProfile1 = () => {
  ...
  const [selectedRoute, setSelectedRoute] = useState<string>('Images1');
  ...
  useEffect(() => {
    const fetchProductImages = async () => {
      ...
      try {
        ...
        if (response.success) {
          setProductImages(response.productBanner); 
        } else {
            ...
            max-lg:mr-3 border-2 border-blue-950 font-bold text-blue-950  h-8 px-2
             bg-white">
               Followed <GrStatusGood size={18} className='text-green-800
                  rounded-full bg-white ml-1 mt-0.5'/>
             </button>
          ) : (
