...
import { bannerOrder } from '@/app/utils/bannerOrders';
...
const Order1 = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      //@ts-ignore
      const response = await bannerOrder(formData);
      if (response.success) {
        setSuccess(true);
     ...
     return (
           <div className=''>
                <div className='flex flex-row py-2 gap-1 mt-5 pl-1 '>
                ...
                <div className='flex flex-row text-slate-950 sm:text-lg text-sm mb-3 gap-2'>
                  <FaRegWindowClose size={24} onClick={closeOrder}/>
                ...
                className='w-80 h-40  max-sm:w-64 p-2 border-2 border-black'
                ...
                <button type="submit" 
                className="flex justify-center w-40 h-7 bg-black hover:bg-green-900
