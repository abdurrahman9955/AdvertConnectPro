...
import { giveVideoRating } from '@/app/utils/rateVideo';
import { useRouter } from 'next/navigation';
import Cookies from 'js-cookie';

const Rating2 = ({ productId }: { productId: string }) => {
  ...
  const handleSubmitRating = async () => {
    try {
      ...
      setIsLoading(true);
      await giveVideoRating(productId, formData);
  ...
  return (
    <div className=''>
      <div className='flex flex-row py-2 gap-1 mt-2 max-sm:mt-3 sm:ml-0 sm:pl-1'>
        {renderStars()}
      </div>

      <button className="bg-blue-950 hover:bg-green-950 max-sm:hidden text-sm
        text-white px-4 mt-2 h-7 max-sm:h-6 rounded"
