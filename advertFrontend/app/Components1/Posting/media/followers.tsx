...
const FollowUser: React.FC<UserProps> = ({ userId }) => {
  ...
  const handleUnfollow = async () => {
   ...
   return (
           <div className=''>
              ...
              <div className='flex flex-row py-2 gap-1 mt-3 max-sm:mt-4  pl-1'>
            
                 {isFollowing ? ( <button onClick={handleUnfollow}
                  className="flex flex-row bg-blue-950 text-sm text-white px-2 h-7 max-sm:px-2 
                  max-sm:h-6 sm:pt-1 rounded">
                  Followed <GrStatusGood size={18} className='text-green-800
                  rounded-full bg-white ml-1 mt-0.5'/> </button> ) : (

                  <button onClick={handleFollow}
                  className="bg-blue-950 text-sm max-sm:px-2 text-white px-4 h-7 max-sm:h-6  rounded">
                  Follow </button> )}  

              </div> 

              <button className="bg-blue-950
                  hover:bg-green-950 ml-4  text-sm
