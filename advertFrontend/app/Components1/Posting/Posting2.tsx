...
/*
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const response: ImageResponse = await getBanners(filter, searchQuery);
        if (response.success && Array.isArray(response.banners)) {
          const fetchedProducts = response.banners;
  ...
  const totalPages = Math.ceil(productsWithImages.length / bannersPerPage);
  ...
  return (
    <div className='font-bold text-sm min-h-screen'>
      {isLoading ? (
        <p className='flex justify-center lg:text-2xl md:text-xl h-screen max-md:text-lg text-blue-950'>
          Loading product Banners...
        </p>
      ) : (
        <>
          {currentProducts.length > 0 && (
            <ul className='grid grid-cols-2 h-auto max-sm:grid-cols-1 max-lg:grid-cols-2 
            max-md:grid-cols-2 gap-1'>
                  ...
                  <div className='flex flex-row flex-wrap justify-between'>
                    ...
                    <p className='overflow-hidden text-xs'>
                      Company Link: {product.companyLink ? (
                        <a href={product.companyLink} target="_blank" rel="noopener noreferrer" className='text-blue-700 ml-5'>
                          {product.companyLink}
                        </a>
                      ) : (
                        " was not provided"
          ...
          {currentProducts.length === 0 && <p className='flex justify-center lg:text-2xl 
          md:text-xl h-screen max-md:text-lg
           text-blue-950'>No products with banner found!</p>}

          <div className='flex justify-center overflow-x-auto mt-4'>
            {Array.from({ length: totalPages }, (_, index) => (
              <button key={index} onClick={() => handlePageChange(index + 1)} 
              className={`mx-1 px-3 py-1 border overflow-x-auto ${currentPage === index + 1 ? 
              'bg-blue-900 text-white rounded-md border border-white' : 
