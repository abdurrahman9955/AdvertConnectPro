...
/*
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const response: ImageResponse = await getImages(filter, searchQuery);
        if (response.success && Array.isArray(response.images)) {
          const fetchedProducts = response.images;
          const sortedProducts = fetchedProducts.sort((a, b) => b.id - a.id); 
  ...
  return (
    <div className='font-bold text-sm min-h-screen'>
      {isLoading ? (
        <p className='flex justify-center lg:text-2xl md:text-xl w-full h-screen 
        ...
        <>
          {currentProducts.length > 0 && (
            <ul className='grid grid-cols-4 max-xl:grid-cols-3 h-auto 
            max-sm:grid-cols-1 max-lg:grid-cols-2 max-md:grid-cols-2 gap-1'>
                ...
                className='border-2 border-slate-950 bg-white p-2'>
                  ...
                  {product.mediaType === 'PHOTO' && (
                    <div className='relative'>
                      <img src={product.mediaUrl} alt={`Product image for
                  ...
                  <div className='flex justify-between flex-wrap'>
                    <button className='flex flex-row bg-blue-50 
                    border border-black rounded-sm h-8 mt-2 p-1'>
                  ...
                  <p className='overflow-hidden text-xs'>
                    Company Name: {product.companyName ? product.companyName : "was not provided"}</p>
                  <p className='overflow-hidden text-xs'>Company Link: {product.companyLink ? <a href={product.companyLink} target="_blank" rel="noopener noreferrer" className='text-blue-700 ml-5'>{product.companyLink}</a> : "was not provided"}</p>
          ...
          {currentProducts.length === 0 && <p className='flex justify-center lg:text-2xl md:text-xl 
          h-screen max-md:text-lg text-blue-950'>No products with images found!</p>}

          <div className='flex justify-center overflow-x-auto mt-4'>
            {Array.from({ length: totalPages }, (_, index) => (
              <button key={index} onClick={() => handlePageChange(index + 1)} 
              className={`mx-1 px-3 py-1 border overflow-x-auto
              ${currentPage === index + 1 ? 'bg-blue-900 text-white border rounded-md border-white' : 
              'bg-white text-black rounded-md border border-black'}`}>
                {index + 1}
              </button>
            ))}
          </div>
