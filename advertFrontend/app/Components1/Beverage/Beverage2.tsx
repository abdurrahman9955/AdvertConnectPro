    ...
    const fetchProducts = async () => {
      try {
        ...
        if (response.success && Array.isArray(response.images)) {
          const groceryProducts = response.images.filter(product => product.product === 'Beverage');
      ...
      {isLoading ? (
        ...
        <>
          {productsWithImages.length > 0 ? (
            ...
            <p className='flex justify-center lg:text-2xl md:text-xl h-screen max-md:text-lg text-blue-950'>No such products recipe found!</p>
          )}
        </>
      )}
    </div>
