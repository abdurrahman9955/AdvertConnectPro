    ...
    const fetchProducts = async () => {
      try {
        ...
        if (response.success && Array.isArray(response.videos)) {
          const groceryProducts = response.videos.filter(product => product.product === 'Babies & Kids');
  ...
  return (
    <div className='font-bold text-sm w-full min-h-screen overflow-hidden' >
      {isLoading ? (
        ...
        <>
          {products.length > 0 && (
            <ul className='grid grid-cols-4 h-auto overflow-y-auto 
            max-sm:grid-cols-1 max-lg:grid-cols-2 max-md:grid-cols-2 
          ...
          No products for babies & kids with video found!
        </p>}
        </>
      )}
    </div>
