    ...
    const fetchProducts = async () => {
      try {
        ...
        if (response.success && Array.isArray(response.images)) {
          const groceryProducts = response.images.filter(product => product.product === 'Babies & Kids');
      ...
      {isLoading ? (
        ...
        <>
          {productsWithImages.length > 0 ? (
            ...
            max-md:text-lg text-blue-950'>No such products  Babies/Kids found!</p>
          )}
        </>
      )}
    </div>
