    ...
    const fetchProducts = async () => {
      try {
        ...
        if (response.success && Array.isArray(response.images)) {
          const groceryProducts = response.images.filter(product => product.product === 'Building Materials');
      ...
      {isLoading ? (
        ...
        <>
          {productsWithImages.length > 0 ? (
            ...
            max-md:text-lg text-blue-950'>No such  products for building found!</p>
          )}
        </>
      )}
    </div>
