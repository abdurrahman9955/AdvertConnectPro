    ...
    const fetchProducts = async () => {
      try {
        ...
        if (response.success && Array.isArray(response.banners)) {
          const groceryProducts = response.banners.filter(product => product.product === 'Medical');
      ...
      {isLoading ? (
        ...
        <>
          {productsWithImages.length > 0 ? (
            ...
            max-md:text-lg text-blue-950'>No medical with banner found!</p>
          )}
        </>
      )}
    </div>
