...
const Recipe4: React.FC = () => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        if (response.success && Array.isArray(response.banners)) {
          const groceryProducts = response.banners.filter(product => product.product === 'Recipe');
  ...
  return (
    <div className='font-bold text-sm min-h-screen overflow-hidden'>
      {isLoading ? (
        <p className='lg:text-2xl md:text-xl h-screen max-md:text-lg text-blue-950'>Loading products...</p>
      ) : (
        <>
          {productsWithImages.length > 0 ? (
            ...
            max-md:text-lg text-blue-950'>No recipe with banner found!</p>
          )}
        </>
      )}
    </div>
