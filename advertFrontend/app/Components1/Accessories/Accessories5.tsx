...
const Accessories5: React.FC = () => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        if (response.success && Array.isArray(response.videos)) {
          const groceryProducts = response.videos.filter(product => product.product === 'Accessories');
  ...
  return (
    <div className='font-bold text-sm w-full min-h-screen overflow-hidden' >
      {isLoading ? (
        <p className='flex justify-center lg:text-2xl md:text-xl h-screen max-md:text-lg
         text-blue-950'>
          Loading products...
        </p>
      ) : (
        <>
                  ...
                  > 
                    ...
                    <div className='flex justify-between flex-wrap'>
                      <button className='flex flex-row bg-blue-50 border
                       border-black h-8 rounded-sm mt-2 p-1'>
          ...
          No accessories with video found!
        </p>}
        </>
      )}
    </div>
